...
def dataprocess():
    sql = sys.argv[1]
    askfilename = sys.argv[2]
    askfileurl = "****\\" + askfilename #处理结果保存的地址

    ansfilename = sys.argv[3]
    ansfileurl = "*****\\" + ansfilename #处理结果保存的地址
    ...
    ####################################################################
    ## 5. 分词 词频统计
    stopwords = pd.read_csv('stop_word.txt', encoding='utf8', names=['stopword'], index_col=False,error_bad_lines=False ,
...
##----------------------------------------------
    ...
    # ask_df.to_csv("ask_df.csv",encoding="utf_8_sig")
    ask_df.to_csv(askfileurl,encoding="utf_8_sig")
    # ans_df.to_csv("ans_df.csv",encoding="utf_8_sig")
    ans_df.to_csv(ansfileurl,encoding="utf_8_sig")
    # return ask_df.to_json(), ans_df.to_json()
    return ask_df, ans_df
