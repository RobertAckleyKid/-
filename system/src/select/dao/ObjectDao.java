...
public class ObjectDao {
	...
	public void addSelectInfo(HttpServletRequest req, Data data, JSONObject json) throws JSONException, SQLException {
		...
		String selectName=data.getParam().has("selectName")? data.getParam().getString("selectName") :null;

		String sql="insert into item_select(username,javaSql,pythonSql,process,selectName,addTime)";
		sql = sql + " values('" + "123" + "',\"" + javaSql + "\",\"" + pythonSql + "\",'before','"+selectName+"','"+str+"')";
	...
	public void changeStatus(Data data,JSONObject json) throws JSONException, SQLException{
		//构造sql语句，根据传递过来的条件参数
		String id=data.getParam().has("id")?data.getParam().getString("id"):null;
		if(id!=null){
			String sql="update item_select";
			sql=sql+" set process='after'";
			sql=sql+" where id="+id;
			data.getParam().put("sql",sql);
			updateRecord(data,json);
		}
