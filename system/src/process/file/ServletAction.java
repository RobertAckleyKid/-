...
import process.dao.Data;
import process.dao.ObjectDao;
...
public class ServletAction extends HttpServlet {
	...
	public void processAction(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (action == null){
			...
			if (action.equals("get_InfoSql")) {
				actionOk=true;
				try {
					getInfo(request, response, json);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}

			if (action.equals("get_InfoInit")) {
				actionOk=true;
				try {
					getInfoInit(request, response, json);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
...
//			}
	...
	private void getInfo(HttpServletRequest request, HttpServletResponse response,JSONObject json) throws JSONException, SQLException {
		ObjectDao dao=new ObjectDao();
		Data data=getPageParameters(request,response,json);
		dao.getInfo(data,json);
	}

	private void getInfoInit(HttpServletRequest request, HttpServletResponse response,JSONObject json) throws JSONException, SQLException {
		ObjectDao dao=new ObjectDao();
		Data data=getPageParameters(request,response,json);
		dao.getInfoInit(data,json);
	}
